# Guides

In-depth guides for FlowGraph features.

## Available Guides

- [Custom Node Renderers](custom-nodes.md) - Create custom node appearances
- [Theming](theming.md) - Customize colors and styles
- [Animations](animations.md) - Add smooth transitions
- [Grouping](grouping.md) - Organize nodes into groups
- [Edge Routing](edge-routing.md) - Configure edge paths
- [Serialization](serialization.md) - Save and load graphs
- [Performance](performance.md) - Optimize for large graphs

## Additional Resources

For full documentation and tutorials, visit [flowgraph.prismify.co](https://flowgraph.prismify.co).
